...
async def list_events_by_tag(request: Request) -> JSONResponse:
    ...
    slug_filter = request.query_params.get("slug_contains")
    payload = [
        {
            "id": event.get("id"),
        ...
        for event in events
        if event.get("id")
        and (not slug_filter or slug_filter in str(event.get("slug") or "").lower())
    ]
    return JSONResponse(payload)
  ...
  <body>
    ...
    <script>
      ...
      async function loadCryptoEvents() {
        ...
        const response = await fetch(`/options/events?tag_id=${tagId}&slug_contains=xrp-updown-15m`);
        if (!response.ok) {
        ...
        options.forEach((event) => {
          ...
          option.textContent = event.slug || event.title || event.id;
